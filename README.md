...
当节点过期或失效时，可以使用 `update_config.py` 脚本自动更新 `config.yaml` 中的代理配置。
...
```sh
python update_config.py [订阅链接]
```

如果未提供订阅链接，脚本将使用内置的默认测试链接。

常用参数：

- `--config <路径>` - 指定要更新的配置文件路径，默认 `config.yaml`
- `--no-backup` - 不生成 `.bak` 备份文件

### 更新内容 [​](#更新内容)

脚本会自动替换 `config.yaml` 中的以下三个字段：
...
> [!TIP]
> 脚本会自动备份原配置文件为 `config.yaml.bak`，如需回滚可手动恢复。

### 重启 Clash [​](#重启-clash)

更新配置后需要重启 Clash 服务使配置生效：
